...
export default function SunFloatingButton() {
  ...
  const initialPos = DEFAULT_POSITIONS.sun;
  ...
  useEffect(() => {
    if (!hasLoadedPosition.current) {
      ...
      getButtonPosition('sun', isUniversePage).then(position => {
  ...
  useEffect(() => {
    if (pathname === '/universe' && universeMode === 'universe') {
      ...
      getButtonPosition('sun', isUniversePage).then(savedPos => {
      ...
      const defaultPos = DEFAULT_POSITIONS.sun;
      Animated.spring(pan, {
        toValue: defaultPos,
        useNativeDriver: false,
      }).start();
  ...
  const handlePress = () => {
    if (pathname === '/morning' || pathname.includes('/(tabs)/morning')) {
      return;
    }
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        Animated.spring(pan, {
          ...
          const isUniversePage = pathname === '/universe';
          saveButtonPosition('sun', { x: boundedX, y: boundedY }, isUniversePage);
        });
